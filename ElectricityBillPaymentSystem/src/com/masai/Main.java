		...
		try {
			do {
				...
				case 4:
					...
					break;
				case 5:
					adminViewAllConsumers(consumers, consumerService);
...
//		String userName = uNamePswdGeneration.usernameGenerate(name);

		Bill bill = new Bill(userName, units, rate, fixedCharge, totalBillAmount, dueDate, status);
...
//		String name = sc.next();
		...
		Bill update = new Bill(userName, units, rate, fixedCharge, totalBillAmount, dueDate, status);
	...
	public static void consumerFunctionality(Scanner sc, Map<String, Consumer> consumers, Map<String, Bill> bills,
		...
		try {
			...
			do {
				...
				switch (choice) {
				case 1:
					consumerViewBill(sc, bills, billService);
					break;
				case 2:
					String result = consumerPayBill(sc, bills, consumers, transactions, consumerService);
				...
				case 3:
					...
					break;
				case 4:
					int walletBalance = consumerViewWalletBalance(userName, consumers, consumerService);
...
//		System.out.println(cons);
	...
	public static void consumerViewBill(Scanner sc, Map<String, Bill> bills, BillService billService)
			throws BillException {
		System.out.println("Enter the userName");
		String userName = sc.next();
		Bill conBill = billService.viewBill(userName, bills);
		System.out.println(conBill);
	}

	public static String consumerPayBill(Scanner sc, Map<String, Bill> bills, Map<String, Consumer> consumers,
			List<Transaction> transactions, ConsumerService consumerService)
			throws InvalidDetailsException, BillException {
		System.out.println("Enter the userName");
		String userName = sc.next();

		System.out.println("Your bill amount is :" + bills.get(userName).getTotalBillAmount());
	...
	public static int consumerViewWalletBalance(String userName, Map<String, Consumer> consumers,
			ConsumerService consumerService) {
		int walletBalance = consumerService.viewWalletBalance(userName, consumers);
		return walletBalance;
	}

	public static void consumerViewMyDetails(String userName, Map<String, Consumer> consumers,
			ConsumerService consumerService) {
		Consumer con = consumerService.viewConsumerDetails(userName, consumers);
		System.out.println("name : " + userName);
		System.out.println("Full Name : " + con.getFirstName() + " " + con.getLastName());
		System.out.println("address : " + con.getAddress());
		System.out.println("mobile number : " + con.getMobileNumber());
		System.out.println("email : " + con.getEmail());
		System.out.println("wallet balance : " + con.getWalletBalance());
	}

	public static void consumerViewConsumerTransactions(String userName, List<Transaction> transactions,
			TransactionService transactionService) throws TransactionException {
		List<Transaction> myTransactions = transactionService.viewConsumerTransactions(userName, transactions);
...
//		checking file
		Map<String, Bill> bills = FileExists.billFile();
